...
void TestPslParserFunction() {
    ...
    LOG(AsString(psl_code));
    
    // Tokenize the PSL code
    PslTokenizer tokenizer(psl_code);
    std::vector<Token> tokens = tokenizer.Tokenize();
    
    LOG("Tokenization completed. Found " + AsString(tokens.size()) + " tokens");
    ...
    LOG("Parsing completed. Found " + AsString(ast.size()) + " nodes");
    ...
    for (PslNode* node : ast) {
        if (CircuitNode* c = dynamic_cast<CircuitNode*>(node)) {
            circuit = c;
            LOG("Found circuit: " + AsString(c->name));
            LOG("Circuit has " + AsString(c->children.size()) + " child nodes");
            break;
        }
    }
