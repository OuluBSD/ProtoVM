#ifndef _ProtoVM_TubeAmpSimulation1980s_h_
#define _ProtoVM_TubeAmpSimulation1980s_h_
...
// Class for tube-based amplifier simulation for 1980s era (high-headroom, tight bass characteristics)
class TubeAmpSimulation1980s : public ElectricNodeBase {
public:
    typedef TubeAmpSimulation1980s CLASSNAME;

    TubeAmpSimulation1980s();
    virtual ~TubeAmpSimulation1980s();

    virtual bool Tick() override;
    virtual String GetClassName() const override { return "TubeAmpSimulation1980s"; }
    ...
    void SetHeadroomMode(bool high_headroom); // Whether to simulate high headroom operation
    ...
    bool GetHeadroomMode() const { return high_headroom_mode; }

private:
    // Amp parameters for 1980s era (high-headroom, tight bass)
    double gain;           // Overall gain (moderate for clean headroom)
    double bass;           // Bass control (with tight, controlled low-end)
    double mid;            // Mid control (often more focused)
    double treble;         // Treble control (clean, extended highs)
    ...
    bool high_headroom_mode;     // Whether to simulate high-headroom operation
    ...
    // Tone stack simulation (classic 1980s style with tight bass)
    ...
    static constexpr double MAX_GAIN = 100.0;  // Moderate gain for headroom
    static constexpr double MIN_BASS = 0.0;
    static constexpr double MAX_BASS = 2.0;    // Extended range
    static constexpr double MIN_MID = 0.0;
    static constexpr double MAX_MID = 2.0;    // Extended range
    static constexpr double MIN_TREBLE = 0.0;
    static constexpr double MAX_TREBLE = 2.5; // Extended high range
    static constexpr double MIN_PRESENCE = 0.0;
    static constexpr double MAX_PRESENCE = 1.8; // Extended range
    static constexpr double MIN_RESONANCE = 0.0;
    static constexpr double MAX_RESONANCE = 1.5; // Extended range
