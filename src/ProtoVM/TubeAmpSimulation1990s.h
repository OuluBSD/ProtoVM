#ifndef _ProtoVM_TubeAmpSimulation1990s_h_
#define _ProtoVM_TubeAmpSimulation1990s_h_
...
// Class for tube-based amplifier simulation for 1990s era (alternative rock characteristics)
class TubeAmpSimulation1990s : public ElectricNodeBase {
public:
    typedef TubeAmpSimulation1990s CLASSNAME;

    TubeAmpSimulation1990s();
    virtual ~TubeAmpSimulation1990s();

    virtual bool Tick() override;
    virtual String GetClassName() const override { return "TubeAmpSimulation1990s"; }
    ...
    void SetAlternativeMode(bool alt_mode); // Whether to simulate alternative rock sound
    ...
    bool GetAlternativeMode() const { return alt_mode; }

private:
    // Amp parameters for 1990s era (alternative rock)
    double gain;           // Overall gain (medium-high for grunge/alternative)
    double bass;           // Bass control (often tight for alternative sound)
    double mid;            // Mid control (often emphasized in alternative rock)
    double treble;         // Treble control (not too bright)
    ...
    bool alt_mode;         // Whether to simulate alternative rock sound
    ...
    // Tone stack simulation (classic 1990s alternative style)
