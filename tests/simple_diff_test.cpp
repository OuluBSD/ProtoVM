#include "../src/ProtoVMCLI/DiffAnalysis.h"
#include "../src/ProtoVMCLI/BehavioralAnalysis.h"
#include "../src/ProtoVMCLI/HlsIr.h"
#include <iostream>
#include <cassert>
...
void TestBehaviorDiff() {
    ...
    if (!diff_result.ok) {
        std::cout << "Error in DiffBehavior: " << diff_result.error_message << std::endl;
        return;
    }
    ...
    // The result can be multiple changes or one of the specific types depending on how we implemented the logic
...
void TestIrDiff() {
    ...
    if (!diff_result.ok) {
        std::cout << "Error in DiffIrModule: " << diff_result.error_message << std::endl;
        return;
    }
